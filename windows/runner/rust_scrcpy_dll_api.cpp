...
RsRegisterClipboardCallbackFn RustScrcpyDllApi::RegisterClipboard(std::string* error) {
  if (!EnsureLoaded(error)) return nullptr;
  if (register_clipboard_ == nullptr) {
    register_clipboard_ = reinterpret_cast<RsRegisterClipboardCallbackFn>(
        ResolveSymbol("rs_register_clipboard_callback", error));
  }
  return register_clipboard_;
}
